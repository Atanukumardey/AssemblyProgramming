.model small

.stack 100h

.data

input_t dw ?  
counter dw ?  ; for loop
isnegative dw ?
input_a dw ?
input_b dw ?

println  db 10,13,"$"

.code  



printString proc
    mov ah, 09h
    int 21h
    ret
    ;end printString

printnewline proc 
    
    lea dx, println
    call printString
     
    ret
    ;end newline

printChar proc
    mov ah, 02h
    int 21h 
    ret

printSpace proc
    mov dl, 32
    call printChar
    
    ret
    ;end printSpace  
    
    
getNum proc
    
    mov ax, 00h
    mov isnegative, ax  ; making flag 0 indecating pobxtive number
    
    mov bx, 0Ah
    mov cx, 00h 
    
    while_getnum:  
        
        ;got the value in al-> AX(ah, al)
        xor ax,ax
        mov ah, 01h
        int 21h
        
        cmp al, 0Dh      ; checking carriage return
        je exit_getNum 
        
        cmp al, 020h ; checking space
        je exit_getNum
        
        ;cmp al, 08h
        ;je handlebackspace
                  
        
        ;for negative number input
        cmp al, 02Dh
        je checkNegative
        
        mov ah, 0 
        sub ax, 030h
        
        push ax
        mov ax, cx
        
        mul bx
         
        mov cx, ax
        pop ax
        add cx,ax
             
        jmp while_getnum
          
   
        exit_getNum:  
        
        ; make negative if negative flag is true
        mov ax, isnegative 
        cmp ax, 1
        je makeNegative
        mov ax, 00h
        mov bx, 00h
        ret 
               
        checkNegative:   
        ; converting the number to negative   
        mov ax, 01h
        mov isnegative, ax ; making flag to 1 indecating negative number
        mov ax, 00h
        jmp while_getnum
        
        makeNegative:
        neg cx
        mov ax, 00h
        mov isnegative, ax
        mov ax, 00h
        jmp exit_getNum  
        
        ;returns value by cx
        ;end getNum
         
             
             
printNum proc 
    
    ;param cx (operates on the value of cx)
                 
    mov dx, 0 
    mov bx, 10
    mov ax, cx
    mov cx, 0  
    
    ;checking negative number
    cmp ax, 0
    jl negativeNumber
    
    cmp ax, 0
    je print_zero 
    
    whilenum:  
        
        cmp ax,0
        je print_printnum
        
        div bx
        
        add dx,48
        
        push dx
        add cx, 1 
               
        xor dx,dx
        
        jmp whilenum 
        
    print_printnum: 
    
        pop dx
        mov ah, 02h
        int 21h
        
        loop print_printnum 

        
    exit_printNum:
        ret 
        
    negativeNumber:    
        push ax
        mov dl, 02Dh 
        mov ah, 02h
        int 21h
        pop ax
        neg ax ; making the number pobxtive   
        mov dx, 00h
        jmp whilenum
        
    print_zero:
        mov dx, 030h
        mov ah, 02h
        int 21h
        
        jmp exit_printNum
        
        
        ;returns void
        ;end printNum    
             

main proc
    mov ax, @data
    mov ds, ax
    
    call getNum
    mov input_t, cx 
    call printnewline

    while_main_out:

        cmp cx, 00h
        jle exit
        
        ;input a and b
        call getNum
        mov input_a, cx 
        call getNum
        mov input_b, cx  
        
        call printnewline
        
        ;input a b end
        
        
        ;check b<=a
        mov ax, 00h
        mov bx, input_a
        cmp cx, bx
        jle printans  ; jump and print ans as 0
        ;
        
        ;now b>a so find b-a and a++
        mov cx, input_b
        sub cx, input_a  ; cx = b-a
        inc input_a      ;a++
        
        ; calculating upparpart multiplication
        mov bx, input_b
        mov ax, input_a
        inc input_a

        while_mul_up:

            cmp input_a, bx
            jg exit_while_mul_up

            mul input_a
            inc input_a

            jmp while_mul_up  
            
        exit_while_mul_up:   
        
        ;storing upper part multiplication in input_a
        mov input_a, ax    
        
        
        mov input_b, cx  ; input_b = cx = (b-a)
        mov cx, 02h
        mov ax, 01h
        
        while_mul_down:

            cmp cx, input_b
            jg exit_while_mul_down

            mul cx
            inc cx

            jmp while_mul_down  
        
        exit_while_mul_down:
        
        mov input_b, ax
        mov dx, 00h
        mov ax, input_a
        div input_b 
        mov bx, 01h
        sub ax, bx
        

        printans:
        mov cx, ax  
        call printNum
        call printnewline

        mov cx, input_t
        dec cx 
        mov input_t, cx 
    
    jmp while_main_out
    
    exit:
    mov ah, 04ch
    int 21h
  
    main endp
end main
    
    